...
HOME_IMAGE_PATH = os.path.join(BASE_DIR, "assets", "home-banner.png")
...
.about-pill.dna-pill .clinical-plus {
    position: absolute;
...
.about-pill.dna-pill .plus-1 { top: 14px; right: 18px; font-size: 1.5rem; }
.about-pill.dna-pill .plus-2 { top: 52px; left: 20px; font-size: 1rem; }
.about-pill.dna-pill .plus-3 { bottom: 16px; right: 34px; font-size: 1.2rem; }
...
if page == "🏠  Home":
    ...
    if os.path.exists(HOME_IMAGE_PATH):
        st.image(HOME_IMAGE_PATH, width=500)
    else:
        st.warning("Home image not found at: assets/home-banner.png")
...
# ─────────────────────────────────────────────────────────────
    ...
    if analyze:
        ...
        if pred is None:
            ...
            with col_detail:
                st.markdown("<div class='key-features-title'><strong>Key Factors in Your Result</strong></div>", unsafe_allow_html=True)
                factors = []
                ...
                st.markdown("<div class='key-features-content'>", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────
    ...
    <div class="about-grid">
        ...
        <div class="about-pill dna-pill">
            <span class="clinical-plus plus-1">+</span>
            <span class="clinical-plus plus-2">+</span>
            <span class="clinical-plus plus-3">+</span>
            <div class="about-pill-icon">🧬</div>
...
# ─────────────────────────────────────────────────────────────
    ...
    steps = [
        ("1", "🧾", "Gather Clinical Data",
         "Collect patient clinical history and available reports before entering values."),
        ("2", "📋", "Fill the Parameters",
         "Enter physical, biochemical, and symptom parameters in the Detection sections."),
        ("3", "🔬", "Upload Ultrasound Images",
         "Add ultrasound and follicle information where available for better prediction."),
        ("4", "🤖", "Run AI Analysis",
         "Click Analyze to process the entered data using the AI model."),
        ("5", "📊", "Review Results",
         "Review risk percentage, key factors, and next-step recommendations."),
    ]
